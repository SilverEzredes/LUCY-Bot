...
async def notify_level_up(message, xp_type, old_lvl, new_lvl):
    if str(message.guild.id) in globals.LEVEL_NOTIF_CHANNEL_IDS:
        ...
        try:
            img.paste(avatar, (15, 5,), avatar)
        except ValueError:
            img.paste(avatar, (15, 5,))
        # Apply base overlay
        img.paste(globals.levelups[xp_type]["overlay"], (0, 0,), globals.levelups[xp_type]["overlay"])
        # Draw old and new level values
        utils.draw_text(draw, globals.font47, f"{old_lvl}", globals.levelups[xp_type]["color"], (444, 65,), 999)
        utils.draw_text(draw, globals.font47, f"{new_lvl}", globals.levelups[xp_type]["color"], (544, 65,), 999)
        # Send image
        binary = io.BytesIO()
